...
namespace images::aos {
 ...
 //Paralelizacion de la funcion to_gray
    void bitmap_aos::to_gray() noexcept {
        const auto max = std::ssize(pixels);
        #pragma omp parallel for
        for (int i = 0; i < max; ++i) {
        pixels[i] = pixels[i].to_gray_corrected();
            //imprimimos el numero de hilos que se han usado
            //std::cout << "Numero de hilos usados: " << omp_get_num_threads() << std::endl;
        }
 ...
 bool bitmap_aos::is_gray() const noexcept {
  ...
  //Paralelizacion de la funcion gauss
 ...
 //Paralelizacion de la funcion generate_histogram
